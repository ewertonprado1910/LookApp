...
export default {
	...
	isValidCreditCard: (creditCard) => {
		if (!valid.cardholderName(creditCard.holder_name).isValid) {
			return { error: true, message: 'Digite um nome válido' };
		}

		if (!valid.number(creditCard.number).isValid) {
			return { error: true, message: ' Digite o número do cartão ' };
		}

		if (!valid.expirationDate(creditCard.valid_date).isValid) {
			return { error: true, message: 'Digite a data do cartão ' };
		}

		if (!valid.cvv(creditCard.cvv).isValid) {
			return { error: true, message: 'Digite o CVV do cartão ' };
		}
		return { error: false };
	},
};
